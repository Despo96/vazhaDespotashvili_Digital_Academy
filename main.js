...
btns.forEach((btn) => {
  btn.addEventListener('click', () => {
    ...
    if (btn.textContent === 'All') {
      cardContainer.innerHTML = cameraItems
        .map((item) => renderItem(item))
        .join('');
      return;
    }

    let filtered = cameraItems.filter(
      (cameraItemsFilter) => cameraItemsFilter.category === btn.textContent
    );
...
const cartTitle = document.querySelector('.cardTitleLength');
const cardBtns = document.querySelectorAll('.addButton');
const cardBtn = [...cardBtns];
...
cardBtn.forEach((card, idx) => {
  card.addEventListener('click', () => {
    cart.push(cameraItems[idx].title);
    cartImg.classList.remove('cartHidden');
    cartTitle.classList.remove('cartNumberHidden');
